...
async function rewriteQueryToEnglish(query) {
  const raw = String(query ?? "").trim();
  if (!raw) {
    semanticDebugLog("[semantic_search] query rewrite skipped: empty query");
    return "";
  }
  if (!containsChinese(raw)) {
    semanticDebugLog("[semantic_search] query rewrite skipped: non-chinese query");
    return "";
  }
  if (!queryRewriteEnabled) {
    semanticDebugLog("[semantic_search] query rewrite skipped: PAGENEXUS_QUERY_REWRITE=0");
    return "";
  }
  if (!apiKey) {
    semanticDebugLog("[semantic_search] query rewrite skipped: PACKY_API_KEY missing");
    return "";
  }
...
async function buildQueryVariants(query) {
  ...
  const deduped = Array.from(new Set(variants.map((item) => item.trim()).filter(Boolean)));
  semanticDebugLog(
    `[semantic_search] query variants source: static=${Math.max(0, variants.length - (rewritten ? 2 : 1))} rewrite=${rewritten ? 1 : 0} final=${deduped.length}`,
  );
  return deduped;
}
